...
def F_cdf(x):
    if x <= -0.2:
        ...
        # F(x) = 2/7 + интеграл от 1 до x от b*(2.5-t) dt
        # = 2/7 + b * [2.5*(x-1) - (x^2-1)/2]
        # После упрощения: F(x) = (2/63) * (-10x^2 + 50x - 31)
        return (2/63) * (-10*x**2 + 50*x - 31)
    else:
        return 1.0

# Часть 3: Обратная функция (квантильная) - ИСПРАВЛЕНО
def F_inv(y):
    if y < 0.0:
        return -0.2
    elif y <= F1:
...
y_vals = f_pdf_vec(x_vals)
plt.plot(x_vals, y_vals, 'r-', linewidth=2.5, label='Теоретическая f(x)')
...
x_vals = linspace(-0.2, 2.5, 300)
y_vals = f_pdf_vec(x_vals)
ax4.plot(x_vals, y_vals, 'r-', linewidth=2.5, label='Теоретическая f(x)')
ax4.set_title('Гистограмма выборки')
ax4.set_xlabel('x')
ax4.set_ylabel('Плотность')
ax4.legend(fontsize=8)
ax4.grid(True, alpha=0.3)
ax4.set_xlim(-0.3, 2.6)
...
# Дополнительная проверка: доля значений на каждом участке
count_1 = sum(1 for x in samples if x <= 1.0)
count_2 = sum(1 for x in samples if 1.0 < x <= 2.5)
print(f"\nРаспределение по участкам:")
print(f"Участок 1 (x ≤ 1.0): {count_1} значений ({count_1/n_samples*100:.1f}%)")
print(f"Участок 2 (1.0 < x ≤ 2.5): {count_2} значений ({count_2/n_samples*100:.1f}%)")
