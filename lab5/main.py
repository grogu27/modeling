...
time = np.arange(min(500, n_steps))
axes[0].plot(time, trajectory1[:500], 'b-', linewidth=0.5)
axes[0].set_ylabel('Состояние')
axes[0].set_title('Цепь Маркова 1 (первые 500 шагов)')
axes[0].grid(True, alpha=0.3)

axes[1].plot(time, trajectory2[:500], 'r-', linewidth=0.5)
axes[1].set_xlabel('Время (шаг)')
axes[1].set_ylabel('Состояние')
axes[1].set_title('Цепь Маркова 2 (первые 500 шагов)')
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('figures/trajectories_comparison.png', dpi=150)
plt.show()
...
plot_autocorrelation_comparison(autocorr1, autocorr2, 
                                'Сравнение автокорреляционных функций', 
                                'autocorrelation_comparison')

# Индивидуальные графики автокорреляции
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
...
axes[0].axhline(y=1.96/np.sqrt(n_steps), color='gray', linestyle='--', linewidth=1, alpha=0.7)
axes[0].axhline(y=-1.96/np.sqrt(n_steps), color='gray', linestyle='--', linewidth=1, alpha=0.7)
...
axes[1].axhline(y=1.96/np.sqrt(n_steps), color='gray', linestyle='--', linewidth=1, alpha=0.7)
axes[1].axhline(y=-1.96/np.sqrt(n_steps), color='gray', linestyle='--', linewidth=1, alpha=0.7)
...
plt.tight_layout()
plt.savefig('figures/autocorrelation_individual.png', dpi=150)
plt.show()
